...
def draw(hists, fits, pros, bgs, ls, dist):
  ...
  if dist is "proton":
    ...
    mu = hists[-1].GetMaximumBin()/bc
    #print mu
    if mu < 0.8 or mu > 1.0:
      mu=0.91
    s = hists[-1].GetBinContent(BinFloor(mu,bc))
    val = hists[-1].GetBinContent(BinFloor(mu-0.05,bc)) - lin*(mu-0.05) - cons
    if val > 0 and s>0:
      sig = math.sqrt(abs(-1/2*1/math.log(val/s)))*(mu-0.05)
    else:
      sig = 0.055
    #print "s="+str(s)+", mu="+str(mu)+", sig="+str(sig)+", cons="+str(cons)+", lin="+str(lin)
    
    axis.SetRange(BinFloor(min_b,bc), BinFloor(max_b,bc))
    #fits[-1].SetParameters(s*2, mu, sig, cons, lin, 0) 
    ...
    fits[-1].SetParLimits(4, 0, 10e6)
    fits[-1].FixParameter(5, 0)
    ...
    #print "S="+str(pars[0])+", mu="+str(pars[1])+", sig="+str(pars[2])+", cons="+str(pars[3])+", lin="+str(pars[4])
...
c.Print("hists/new/Q2W_2D/out2D_Q2W_total.pdf")
...
for zoom in ["total", "proton"]:
  for sec in range(6):
    ...
    c.Print("hists/new/Q2W_2D/out2D_Q2W_"+zoom+"_sec"+str(sec+1)+".pdf")
...
hTotal2D.GetYaxis().SetTitle("Q^{2} (GeV^{2})")
hTotal2D.Draw("COLZ")
c.Print("hists/new/Q2W_2D/out2D_Q2W_proton.pdf")
